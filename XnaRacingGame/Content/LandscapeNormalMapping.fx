...
float4 PS_DiffuseWithDetail(VertexOutput_Detail In) : COLOR
{
	...
	detailTexture = (2.0 * detailTexture);// - 0.75;
	//tst:
	//float4 detailTexture2 = tex2D(detailTextureSampler, In.detailTexCoord*16);
	//detailTexture = (4.0 * detailTexture * detailTexture2);// - 0.75;
...
technique DiffuseWithDetail
{
	pass P0
	{
		...
		//PixelShader  = compile ps_1_1 PS_DiffuseWithDetail();
		sampler[0] = (diffuseTextureSampler);
		sampler[1] = (normalTextureSampler);
