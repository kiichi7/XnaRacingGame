// Project: RacingGame, File: Input.cs
// Namespace: RacingGame.GameLogic, Class: Input
// Path: C:\code\RacingGame\GameLogic, Author: Abi
// Code lines: 1038, Size of file: 27,41 KB
// Creation date: 24.09.2006 09:28
// Last modified: 07.11.2006 02:59
...
namespace RacingGame.GameLogic
{
	...
	class Input
	{
		...
		/// <summary>
		/// Create input, prevent instantiation.
		/// </summary>
		private Input()
		{
		} // Input()
		...
		public static void TestKeyboardChatInput()
		{
			/*first try, suxx
			string chatText = "";
			List<Keys> keysLastFrame = new List<Keys>();

			TestGame.Start("TestKeyboardChatInput",
				null,
				delegate
				{
					Keys[] keysNow = Keyboard.GetPressedKeys();
					foreach (Keys key in keysNow)
						if (keysLastFrame.Contains(key) == false)
							chatText += key.ToString();
					keysLastFrame = new List<Keys>(keysNow);

					Thread.Sleep(100);
				},
				null);
			Log.Write("Chat text=" + chatText);
			 */
		...
		public static void TestGamePadInput()
		{
			...
			TestGame.Start("TestGamePadInput",
				null,
				delegate
				{
					TextureFont.WriteText(2, 30,
						"Got gamepad: " + Input.IsGamePadConnected);
					currentSpeed +=
						-Input.GamePad.Triggers.Left * 2 +
						Input.GamePad.Triggers.Right +
						Input.GamePad.ThumbSticks.Right.Y +
						(Input.GamePad.DPad.Up == ButtonState.Pressed ? -1 : 0) +
						(Input.GamePad.DPad.Down == ButtonState.Pressed ? 1 : 0);
					TextureFont.WriteText(2, 60,
						"Current speed: " + currentSpeed);
					steering +=
						Input.GamePad.ThumbSticks.Left.X +
						(Input.GamePad.DPad.Left == ButtonState.Pressed ? -1 : 0) +
						(Input.GamePad.DPad.Right == ButtonState.Pressed ? 1 : 0);
