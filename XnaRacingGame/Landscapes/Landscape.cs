// Project: RacingGame, File: Landscape.cs
// Namespace: RacingGame.Landscapes, Class: LandscapeObject
// Path: C:\code\RacingGame\Landscapes, Author: Abi
...
namespace RacingGame.Landscapes
{
	...
	public class Landscape : IDisposable
	{
		...
		} // ReplaceStartLightObject(number)
		...
		#region Unit Testing
#if DEBUG
		#region Generate landscape height file
#if !XBOX360
		/// <summary>
		...
		/// the Xbox360 does not support the Texture.GetData method.
		/// </summary>
		public static void GenerateLandscapeHeightFile()
		{
			TestGame.Start(
				delegate
				{					
					...
					Texture2D tex =
						...
						//Path.Combine(Directories.ContentDirectory, "LandscapeGridHeights"));
						//*old
						// Note: Doesn't work on the Xbox360!
						Texture2D.FromFile(BaseGame.Device,
						"Landscapes\\"+"LandscapeGridHeights.png");
						//*/
					...
					for (int x = 0; x < GridWidth; x++)
						for (int y = 0; y < GridHeight; y++)
						{
							// Flip x and y to make loading easier later
							int index = y + x * GridHeight;
							heights[x + y * GridWidth] = texData[index].R;
						} // for for

					// And finally save it
					FileStream file = File.Create(
						"Content\\"+LandscapeHeightsDataFilename);
					file.Write(heights, 0, GridWidth*GridHeight);
					file.Close();
					// That's it already!
				},
				null);
		...
		public static void TestRenderLandscape()
		{
			TestGame.Start(
				...
				delegate
				{
					RacingGameManager.LoadLevel(
						//RacingGameManager.Level.Beginner);
						RacingGameManager.Level.Advanced);
						//RacingGameManager.Level.Expert);
					RacingGameManager.Landscape.SetCarToStartPosition();
				},
				delegate
				{
					...
					TextureFont.WriteText(
						...
						RacingGameManager.Landscape.landscapeObjects.Count);
				});
		} // TestRenderLandscape()
		#endregion
#endif
