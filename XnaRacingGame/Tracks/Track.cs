// Project: RacingGame, File: Track.cs
// Namespace: RacingGame.Tracks, Class: Track
// Path: C:\code\RacingGame\Tracks, Author: Abi
// Code lines: 1432, Size of file: 48,57 KB
// Creation date: 29.09.2006 08:47
// Last modified: 04.11.2006 06:39
...
namespace RacingGame.Tracks
{
	...
	/// from 3ds max as collada files (see TrackData class).
	/// </summary>
	public class Track : TrackLine, IDisposable
	{
		...
		/// the road, which is defined in	TrackVertex.RoadWidthScale.
		/// </summary>
		const float RoadTunnelTextureWidthFactor = 0.25f;//0.75f;
		...
		/// </summary>
		const float SignGap = 24;//33.0f;//25.0f;
		#endregion
		...
		Material guardRailMaterial = new Material(
			//Color.Black,//new Color(162, 162, 162),
			//Color.Black,//new Color(90, 90, 90),
			new Color(72, 72, 72),//new Color(90, 90, 90),
			new Color(182, 182, 182),
			new Color(225, 225, 225),
			"Leitplanke%0", "LeitplankeNormal%0", "", "");

		/*unused, now rendering objects is done in landscape!
		/// <summary>
		/// Guard rail holder model for the left and right sides of the road.
		/// </summary>
		Model guardRailHolderModel = new Model("GuardRailHolder");
		/// <summary>
		/// Column segment for the landscape ground (just a helper object,
		/// the column itself is generated!)
		/// </summary>
		Model columnHolderModel = new Model("RoadColumnSegment");
		 */
    ...
    TangentVertex[] roadBackVertices = null;
...
#if !XBOX360
			// Crashes the Xbox 360, on Windows it works fine (see documentation,
			// it will use linear if anisotropic is not supported!
			// On the Xbox 360 however it crashes somewhere random in the code,
			// much later in the frame than this call occurs.
			//BaseGame.Device.SamplerStates[0].MipFilter = TextureFilter.Anisotropic;
#endif
		...
		public void GenerateShadow()
		{
			...
			// Generate shadows for both rails (skip for ps11, too slow and blocky)
		...
		} // GetTrackPositionMatrix(trackPositionPercent, roadWidth, nextRoadWid)
		...
		public bool IsTunnel(int trackSegment)
		{
			...
			return false;
		} // IsTunnel(trackSegment)
		#endregion

		#region Unit Testing
#if DEBUG
		#region Test render track
		/// <summary>
		/// Test render track
		/// </summary>
		//[Test]
