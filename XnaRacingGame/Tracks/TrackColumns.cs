// Project: RacingGame, File: TrackColumns.cs
// Namespace: RacingGame.Tracks, Class: TrackColumns
// Path: C:\code\RacingGame\Tracks, Author: Abi
// Code lines: 449, Size of file: 14,28 KB
// Creation date: 30.09.2006 23:44
// Last modified: 13.10.2006 16:13
...
namespace RacingGame.Tracks
{
	...
	class TrackColumns : IDisposable
	{
		...
		/// </summary>
		const float ColumnsDistance = 33.0f;//50.0f;//100.0f
		...
		/// </summary>
		const float TopColumnSubHeight = 0.55f;//0.75f;

		/*unused, see below for logic
		/// <summary>
		/// Use up to 45 degrees of the road up/down angle to generate columns.
		/// Everything above this angel (e.g. loopings) doesn't get columns.
		/// </summary>
		const float MaxColumnGenerationAngel = MathHelper.Pi / 2.0f;
		 */
		#endregion
		...
		public void Render(Material columnMaterial)
		{
			...
			// Restore the world matrix
			BaseGame.WorldMatrix = Matrix.Identity;
		} // Render(guardRailMaterial, holderModel)
		...
		#region Generate and use shadow for the columns
		// Currently disabled in Landscape rendering
#if UNUSED
		/// <summary>
		/// Generate shadow
		/// </summary>
		...
		{
			// Just render out the columns (world matrix is already set)
			RenderColumnVertices();
		} // GenerateShadow()
